...
# https://github.com/Xavierhuang/LingCode/issues/14371
#
...
OPENCODE_DIR="${OPENCODE_DIR:-$HOME/.lingcode}"
OPENCODE_SRC="${OPENCODE_SRC:-$HOME/.lingcode-src}"
...
PROMPT_FILE="$OPENCODE_SRC/packages/lingcode/src/cli/cmd/tui/component/prompt/index.tsx"
...
cd "$OPENCODE_SRC/packages/lingcode"
...
BUILT_BINARY="$OPENCODE_SRC/packages/lingcode/dist/opencode-${PLATFORM}-${ARCH}/bin/opencode"

if [ ! -f "$BUILT_BINARY" ]; then
  BUILT_BINARY=$(find "$OPENCODE_SRC/packages/lingcode/dist" -name "lingcode" -type f -executable 2>/dev/null | head -1)
fi
...
if [ -f "$BUILT_BINARY" ]; then
  ...
  echo "  cd $OPENCODE_SRC/packages/lingcode && bun run build --single"
  exit 1
fi
...
echo "  curl -fsSL https://lingcode.dev/install | bash"
echo ""
