...
import { createOpencodeClient } from "@lingcode-ai/sdk"
...
function useShareUrl() {
  return isMock() ? "https://dev.lingcode.dev" : "https://lingcode.dev"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.lingcode.dev/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    response = await fetch("https://api.lingcode.dev/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "lingcode-agent[bot]"`
  await $`git config --global user.email "lingcode-agent[bot]@users.noreply.github.com"`
}
