...
const repo = process.env.GH_REPO ?? "Xavierhuang/LingCode"
const bot = ["actions-user", "github-actions[bot]", "lingcode", "lingcode-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/lingcode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/lingcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/lingcode/")) areas.add("core")
