...
import { Catalog } from "@lingcode-ai/core/catalog"
import { Location } from "@lingcode-ai/core/location"
import { ModelV2 } from "@lingcode-ai/core/model"
import { PluginV2 } from "@lingcode-ai/core/plugin"
import { OpencodePlugin } from "@lingcode-ai/core/plugin/provider/opencode"
import { ProviderV2 } from "@lingcode-ai/core/provider"
...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and cancels paid models without credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const updated = yield* plugin.trigger("provider.update", {}, { provider: provider("lingcode"), cancel: false })
        const paid = yield* plugin.trigger(
          "model.update",
          {},
          { model: model("lingcode", "paid", { cost: cost(1) }), cancel: false },
        )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        yield* plugin.trigger("provider.update", {}, { provider: provider("lingcode"), cancel: false })
        const free = yield* plugin.trigger(
          "model.update",
          {},
          { model: model("lingcode", "free", { cost: cost(0) }), cancel: false },
        )
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const outputOnly = yield* plugin.trigger(
          "model.update",
          {},
          { model: model("lingcode", "output-only", { cost: cost(0, 1) }), cancel: false },
        )
    ...
    withEnv({ OPENCODE_API_KEY: undefined, CUSTOM_OPENCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        ...
        const updated = yield* plugin.trigger(
          "provider.update",
          {},
          { provider: provider("lingcode", { env: ["CUSTOM_OPENCODE_API_KEY"] }), cancel: false },
        )
        const paid = yield* plugin.trigger(
          "model.update",
          {},
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const updated = yield* plugin.trigger(
          "provider.update",
          {},
          { provider: provider("lingcode", { enabled: { via: "auth", service: "lingcode" } }), cancel: false },
        )
        const paid = yield* plugin.trigger(
          "model.update",
          {},
  ...
  it.effect("prefers gpt-5-nano as the opencode small model", () =>
    Effect.gen(function* () {
      ...
      const providerID = ProviderV2.ID.lingcode
