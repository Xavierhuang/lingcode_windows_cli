...
// Synthetic LingModel provider — injected into every catalog response.
// Not in models.dev (we don't own that catalog); injected locally so
// `lingcode providers list` always offers it. Routes Anthropic Messages API
// requests to lingcode.dev's proxy at /api/inference/anthropic/v1/messages
// with the user's lcat_* token attached by @ai-sdk/anthropic.
// Per project branding policy: never name the upstream vendor in any
// user-visible string here. "LingModel" or generic "your API key" only.
const LINGMODEL_PROVIDER: Provider = {
  ...
  // Anthropic-compatible upstream the operator currently has configured
  // (DeepSeek's /anthropic, Moonshot/Kimi's /anthropic, etc. — opaque to
...
export const layer: Layer.Layer<Service, never, Requirements> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Merge the local LingModel provider into every catalog read. Putting
    // it in `get()` rather than mutating the cached populate result means
    // refresh() still re-fetches from models.dev cleanly; lingmodel rides
    // on top.
    const get = (): Effect.Effect<Record<string, Provider>> =>
      cachedGet.pipe(Effect.map((catalog) => ({ ...catalog, lingmodel: LINGMODEL_PROVIDER })))
