...
- The server still lives in `packages/lingcode`.
...
Until enough shared service code lives outside `packages/lingcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/lingcode`

It should not import `packages/lingcode` services while `packages/lingcode`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/lingcode` implementation details.
