...
- Author package entrypoint is `@lingcode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://lingcode.dev/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@lingcode-ai/plugin/tui`.
- `@lingcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@lingcode-ai/plugin/tui"
...
- If `engines.lingcode` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.lingcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.lingcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.lingcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.lingcode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@lingcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@lingcode-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@lingcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"lingcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Local plugins persist installed themes under the local `.lingcode/themes` area near the plugin config source.
...
- Local smoke plugin: `.lingcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.lingcode/plugins/tui-vim.tsx`
- Local smoke config: `.lingcode/tui.json`
- Local smoke theme: `.lingcode/plugins/smoke-theme.json`
