...
Delete `packages/lingcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/lingcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/lingcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/lingcode`
- TUI plugin loader tests in `packages/lingcode` if runtime plugin API wiring changed
