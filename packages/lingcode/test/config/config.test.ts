...
import { EffectFlock } from "@lingcode-ai/core/util/effect-flock"
...
import { CrossSpawnSpawner } from "@lingcode-ai/core/cross-spawn-spawner"
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://lingcode.dev/config.json",
      snapshot: false,
    })
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    expect(content).toContain('"$schema": "https://lingcode.dev/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["lingcode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
...
test("loads config from .lingcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lingcode")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".lingcode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .lingcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lingcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .lingcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lingcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .lingcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lingcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({ $schema: "https://lingcode.dev/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .lingcode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".lingcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .lingcode config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .lingcode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          $schema: "https://lingcode.dev/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".lingcode")
...
test("local .lingcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .lingcode directory config enables it
      const opencodeDir = path.join(dir, ".lingcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.lingcode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.lingcode/plugin/demo.ts", "file:///project/.lingcode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.lingcode/plugin/demo.ts"])
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .lingcode/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .lingcode directory with a command
          const opencodeDir = path.join(dir, ".lingcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await withTestInstance({
        ...
        fn: async (ctx) => {
          ...
          // Project .lingcode should NOT be in directories list
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.lingcode.managed.test",
          PayloadType: "ai.lingcode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://lingcode.dev/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://lingcode.dev/config.json")
})
