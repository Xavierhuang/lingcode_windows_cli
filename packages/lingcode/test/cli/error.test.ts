...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      ...
      },
      {
        ...
        data: { path: "/tmp/opencode.jsonc", dir: ".lingcode", suggestion: "lingcode" },
        expected:
          'Directory ".lingcode" in /tmp/opencode.jsonc is not valid. Rename the directory to "lingcode" or remove it. This is a common typo.',
      },
      {
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.lingcode.dev/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.lingcode.dev/auth/device/code.")
