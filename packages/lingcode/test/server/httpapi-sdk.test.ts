...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".lingcode", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("lingcode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("lingcode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
