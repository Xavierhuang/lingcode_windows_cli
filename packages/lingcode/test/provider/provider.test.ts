...
async function markPluginDependenciesReady(dir: string) {
  ...
  await Bun.write(
    ...
    JSON.stringify({ packages: { "": { dependencies: { "@lingcode-ai/plugin": "0.0.0" } } } }),
  )
}

function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("lingcode")]
...
test("ModelNotFoundError suggests catalog models for unloaded providers", async () => {
  ...
  await withTestInstance({
    ...
    fn: async (ctx) => {
      ...
      try {
        await getModel(ProviderID.lingcode, ModelID.make("claude-haiku-fake-model"), ctx)
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "lingcode" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".lingcode")
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".lingcode", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
