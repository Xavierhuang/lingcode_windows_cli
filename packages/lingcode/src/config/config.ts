...
import { InstallationLocal, InstallationVersion } from "@lingcode-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@lingcode-ai/core/schema"
...
export const Info = Schema.Struct({
  ...
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://lingcode.dev/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://lingcode.dev/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@lingcode-ai/core/schema`. See the definition
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://lingcode.dev/config.json"
          ...
          yield* fs
            .writeWithDirs(
              ...
              JSON.stringify(
                {
                  $schema: "https://lingcode.dev/config.json",
                  // LingCode default agent config. Managed LingModel is the default
                  // model (works once the user signs in / has a token); otherwise the
                  // agent falls back to the first available provider. The provider list
                  // is curated to LingCode's supported set so the picker mirrors the
                  // LingCode Mac IDE rather than the full 140-provider catalog. Each
                  // provider surfaces in the agent once its API key is configured
                  // (via `lingcode providers`); LingModel needs no key. Edit this file
                  // to add or remove providers.
                  model: "lingmodel/lingmodel-standard",
                  enabled_providers: [
                    "lingmodel",
                    "opencode",
                    "anthropic",
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://lingcode.dev/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://lingcode.dev/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("lingcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".lingcode") || dir === Flag.OPENCODE_CONFIG_DIR) {
          ...
          // Auto-discovered plugins under `.lingcode/plugin(s)` are already local files, so ConfigPlugin.load
