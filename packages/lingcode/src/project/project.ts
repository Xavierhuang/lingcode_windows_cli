...
import { NonNegativeInt, optionalOmitUndefined } from "@lingcode-ai/core/schema"
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(pathSvc.join(dir, "lingcode")).pipe(
        Effect.map((x) => x.trim()),
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        if (!id) {
          ...
          if (id) {
            yield* fs.writeFileString(pathSvc.join(common, "lingcode"), id).pipe(Effect.ignore)
          }
        }
