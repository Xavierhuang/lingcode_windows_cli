...
import { ensureProcessMetadata } from "@lingcode-ai/core/util/lingcode-process"
...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    // Fire-and-forget anonymous heartbeat (no await, no error surface).
    // Throttled to once per 23h via local state at ~/.config/lingcode/telemetry.json.
    // Opt out with `lingcode telemetry off` (TODO: wire that subcommand).
    if (!Installation.isLocal()) {
      void import("./telemetry").then((t) =>
        t.sendHeartbeatIfDue(InstallationVersion).catch(() => undefined),
      )
    }
