...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@lingcode-ai/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `lingcode auth login` in the terminal",
      name: "Login with LingCode",
      id: "opencode-login",
    }
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const opencodeProvider = providers.find((p) => p.id === "lingcode")
  if (opencodeProvider) {
