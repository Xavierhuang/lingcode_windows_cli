This is a plugin to simulate a remote environment locally. Add this to `.lingcode/opencode.jsonc`:

```json
  "plugin": ["../packages/lingcode/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/lingcode/script/run-workspace-server
```
