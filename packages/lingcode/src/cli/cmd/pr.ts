...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run lingcode",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          const importResult = yield* Effect.promise(() =>
            Process.text(["lingcode", "import", sessionUrl], { nothrow: true }),
          )
    ...
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["lingcode", ...lingcodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
