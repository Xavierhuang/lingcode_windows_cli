...
import { Hash } from "@lingcode-ai/core/util/hash"
import { Plugin } from "../plugin"
import { type LanguageModelV3 } from "@ai-sdk/provider"
import * as ModelsDev from "@lingcode-ai/core/models"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@lingcode-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
  ...
  return {
    ...
    opencode: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["lingcode"]?.options?.apiKey)
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "lingcode",
          },
        },
      }),
    kilo: () =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      if (providerID.startsWith("lingcode")) {
        priority = ["gpt-5-nano"]
      }
