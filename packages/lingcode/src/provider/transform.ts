...
import type * as ModelsDev from "@lingcode-ai/core/models"
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "lingcode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("lingcode")) {
      result["promptCacheKey"] = input.sessionID
