...
Replace runtime Bun shell template-tag usage in `packages/lingcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/lingcode/src` is removed except:
