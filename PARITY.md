...
| serve HTTP | ~40% | This CLI exposes a bigger REST API; Swift has better sandboxing + auth |
| auth / providers | Partial | Same token + env var, **incompatible on-disk storage** |
| mcp | Partial | Swift has a registry; this CLI has OAuth. **No `lingcode-memory` tool here** |
...
| Concurrency | `--max-concurrent` (429 on overflow) | no explicit limiter |
| CORS | `--web-origin` (repeatable) | `--cors` (repeatable) + mDNS |
| Service discovery | ❌ | ✅ mDNS |

**Swift leads:** bearer auth, workspace-root sandboxing, concurrency limiter.
...
| `lingcode-memory` tool | ✅ present (memory_save / skill_propose / session_search) | ❌ **absent** (STATUS.md Phase 4) |

Neither acts as an MCP **server**. The `lingcode-memory` MCP tool is a confirmed
Mac-only gap (this CLI defers it to Phase 4 / v0.9.1+).
...
## Gaps closed on Linux/Windows (2026-06-19)
...
- `serve --max-concurrent` 429 limiter — needs a universal router middleware; not yet wired
...
*Generated by cross-codebase exploration on 2026-06-19. Line-level refs were accurate
at generation time; re-verify against current source before relying on them.*
