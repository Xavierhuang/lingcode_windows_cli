...
## Gaps closed on Linux/Windows (2026-06-20)

Implemented in this CLI to match the Swift behavior, by release:

**rc17**
- **RTK (token killer)** — conservatively compacts verbose bash output before it
  reaches the model (identical consecutive lines → `[×N]`, blank-run collapse,
  trailing-whitespace trim); information-preserving and idempotent, never grows
  the input. **Implemented natively** (not a Rust sidecar) so it's compiled into
  every one of the 12 platform binaries and is guaranteed on for *every* user
  (opt out `LINGCODE_RTK=0`) — a cross-compiled sidecar could silently no-op
  where the build/embed/exec fails. Verified: 500-line log 10505 b → 34 b;
  non-repetitive output untouched; full original still saved to the truncation
  file. (`tool/rtk.ts`, hooked in `tool/shell.ts`, `LINGCODE_RTK` flag)
...
- `lingcode build` — Cosine/Genie-style autonomous bootstrap (the one remaining
  closeable gap; ~5–7 days). Plan: `docs/BUILD_RTK_PLAN.md`.
...
*Generated by cross-codebase exploration on 2026-06-19; refreshed 2026-06-20 through
v0.9.0-rc17 (rc15: memory tools, auth multi-account + encrypted export/import, serve
bearer/`--workspace-root`; rc16: `serve --max-concurrent`; rc17: RTK token killer). The
only remaining closeable gap is `lingcode build`. Line-level refs were accurate at refresh
